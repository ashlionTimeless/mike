...
/**
 * Structured agent-step logger. Each step is sent to agent-log-writer POST /log-step.
 */
export class AgentStepLogger {
  ...
  constructor(args: { userId?: string; model?: string }) {
    ...
    if (!remoteConfig) {
      throw new Error(
        "AGENT_LOG_WRITER_URL must be set when AGENT_STEP_LOGGING is enabled. " +
          "Agent step logs are written via the agent-log-writer /log-step endpoint.",
      );
    }
  ...
  getRunId(): string {
    return this.remoteWriter?.getRunId() ?? "";
  }

  getLogFilename(): string {
    return this.remoteWriter?.getLogFilename() ?? `agent-${this.sessionId}.jsonl`;
  }
